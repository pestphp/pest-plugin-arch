    ...
    public static function userNamespaces(): array
    {
        ...
        foreach (self::loader()->getPrefixesPsr4() as $namespace => $directories) {
            foreach ($directories as $directory) {
                ...
                if (
                    str_starts_with($directory, $rootPath.'tests')
                    && ! (
                        str_ends_with($directory, 'pest-plugin-arch'.DIRECTORY_SEPARATOR.'tests')
                        || str_contains($directory, 'pest'.DIRECTORY_SEPARATOR.'tests')
                    )
                ) {
                    continue;
                }
